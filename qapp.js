// Sample questions (you can expand or fetch dynamically)
const questions = [
  {
...
document.querySelectorAll('.categories .category').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.categories .category').forEach(b => b.classList.remove('active'));
...
// Start quiz logic
function startQuiz() {
  const categoryData = questions.find(q => q.category === selectedCategory.toLowerCase());
  ...
  quizQuestions = shuffleArray([...categoryData.questions]).slice(0, totalQuestions);
  currentIndex = 0;
  score = 0;
...
function renderQuestion() {
  ...
  questionStatus.textContent = `Question ${currentIndex + 1} of ${quizQuestions.length}`;
...
function endQuiz() {
  ...
  quizQuestions.forEach(q => {
    if (q.selected === undefined) {
      unanswered++;
    } else if (q.selected === q.correctAnswer) {
      correct++;
    } else {
      wrong++;
    }
...
if (percentage >= 75) {
  ...
  performance.textContent = "Better";
} else {
  performance.textContent = "Bad";
}
